...
export async function getCategorySpend(category: Category, providerId: string) {
    ...
    return {success: true, message: 'Bills found for this category', data: spend._sum.totalAmount || 0}
}
...
export async function getFilteredBills({ from, to, categories, providerId }: {
    from?: string,
    to?: string,
    categories?: string[],
    providerId: string
...
export async function getFilteredBillsService(filters: {
    ...
    const {
      providerId, from, to, categories, search,
      sortBy = 'purchaseDate',
      sortOrder = 'desc',
      page = 1,
      limit = 10
